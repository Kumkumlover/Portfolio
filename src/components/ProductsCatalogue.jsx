...
import { FadeInUp, StaggerContainer, staggerChild, useCardGlow, motion } from '../lib/animations';

export default function ProductsCatalogue() {
  const glowCard2 = useCardGlow();
  const glowCard3 = useCardGlow();
      ...
      <div className="max-w-[1200px] mx-auto">
        ...
        <StaggerContainer className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <motion.div 
            ...
            className="bg-primary text-canvas-light rounded-[20px] p-[32px] flex flex-col h-full hover:bg-primary-deep transition-colors duration-200"
          >
          ...
          <motion.div 
            ref={glowCard2.ref}
            onMouseMove={glowCard2.handleMouseMove}
            onMouseLeave={glowCard2.handleMouseLeave}
            variants={staggerChild}
            className="relative bg-canvas-light border border-hairline-light rounded-[20px] p-[32px] flex flex-col h-full hover:border-hairline-strong hover:-translate-y-0.5 hover:scale-[1.02] transition-all duration-300"
          >
            <div className="absolute inset-0 rounded-[20px] pointer-events-none" style={{ background: 'radial-gradient(400px circle at var(--glow-x, 50%) var(--glow-y, 50%), rgba(0,102,255,0.06), transparent 60%)', opacity: 'var(--glow-opacity, 0)', transition: 'opacity 0.3s ease' }} />
          ...
          <motion.div 
            ref={glowCard3.ref}
            onMouseMove={glowCard3.handleMouseMove}
            onMouseLeave={glowCard3.handleMouseLeave}
            variants={staggerChild}
            className="relative bg-surface-soft border border-hairline-light rounded-[20px] p-[32px] flex flex-col h-full hover:border-hairline-strong hover:-translate-y-0.5 hover:scale-[1.02] transition-all duration-300"
          >
            <div className="absolute inset-0 rounded-[20px] pointer-events-none" style={{ background: 'radial-gradient(400px circle at var(--glow-x, 50%) var(--glow-y, 50%), rgba(0,102,255,0.06), transparent 60%)', opacity: 'var(--glow-opacity, 0)', transition: 'opacity 0.3s ease' }} />
            <h3 className="text-[32px] font-medium leading-[1.19] tracking-[-0.32px] mb-4 text-ink">
              LILA BLACK
